				...
				if(check < date2) {
					...
					$.ajax({
						...
						success: function(data){
							console.log("컨트롤러에서 넘어온 값", data.result);
							if(data.result != -1) {
				...
				<c:forEach var="result" items="${reserList}">
					...
					<form action="/project_trip/trip">
						<tr>
							...
							<td><input type="button" class="del" data-checkout="${result.reserve_checkout}" data-num="${result.reserve_no}"  value="❌"></td>
						</tr>
						<tr>
						...
						<tr>
							<td>체크인 : <span class="checkin" >${result.reserve_checkin}</span> / 체크아웃 : ${result.reserve_checkout}
							</td>
						</tr>
						<tr>
